...
public class HospitalAdapter extends RecyclerView.Adapter<HospitalAdapter.ViewHolder> {
    ...
    public  HospitalAdapter(Context context, List<Hospital> hospitals,  OnItemClickListener listener){
        ...
        this.allHospitals = new ArrayList<>(hospitals);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Hospital hospital = hospitals.get(position);
         ...
         holder.txtName.setText("+" + hospital.getRating());
    ...
    public void filter(String query) {
        if (query.isEmpty()) {
            hospitals = new ArrayList<>(allHospitals);
        } else {
            List<Hospital> filtered = new ArrayList<>();
            for (Hospital h : allHospitals) {
                if (h.getName().toLowerCase().contains(query.toLowerCase()) || h.getAddress().toLowerCase().contains(query.toLowerCase())) {
                    filtered.add(h);
                }
            }
            hospitals = filtered;
    ...
    public void updateData(List<Hospital> newHospitals) {
        this.hospitals = new ArrayList<>(newHospitals);
